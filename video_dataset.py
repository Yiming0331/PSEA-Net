...
from add_distortion_to_video import get_distortion_function, get_distortion_parameter
try:
    import lmdb
...
class VideoDataSet(data.Dataset):
    ...
    def __init__(self, root_path, list_file, num_groups=64, frames_per_group=1, sample_offset=0, num_clips=1,
                 ...
                 image_tmpl='{:03d}.jpg', transform=None, is_train=True, test_mode=False, seperator=' ',
        ...
        # self.max_frame_idx = 0  # Initialize max_frame_idx
    ...
    def _parse_list(self):
        ...
        for x in open(self.list_file):
            ...
            #--------------------  add --------------------
            try:
                img_file_names = os.listdir(os.path.join(self.root_path, elements[0]))
                img_file_names = sorted([os.path.splitext(img_name)[0]  for img_name in img_file_names])
               
                if len(img_file_names) < self.video_length:
                    self.video_length = len(img_file_names)
                end_frame = min(len(img_file_names), end_frame)
                elements[2] = end_frame
                # if "311_387" in elements[0]:
                #     print(f"file_nums:{len(img_file_names)},txt file{self.list_file}, path: {elements[0]},start_frame{start_frame}, end_frame:{end_frame}")
        ...
        self.max_frame_idx = max([v.end_frame for v in video_list])
    ...
    def _get_valid_frame_indices(self, directory, start_idx):
        valid_indices = []
        current_idx = start_idx
        # num_skipped_frames = 0
        consecutive_failures = 0  # 用于记录连续失败的次数  
        while len(valid_indices) < self.num_consecutive_frames and consecutive_failures < 5:
                if current_idx > self.max_frame_idx:
                    break
                image_path = os.path.join(self.root_path, directory, self.image_tmpl.format(current_idx))
        ...
        # 如果没有足够的帧，用最后一帧补充
        while len(valid_indices) < self.num_consecutive_frames:
            if valid_indices:  # 确保 valid_indices 不是空的
    ...
    def get_data(self, record, indices):
        ...
        if self.modality == 'sound':
            ...
            for seg_ind in indices:
                ...
                # new_seg_ind_img = self._get_valid_frame_indices(record.path, seg_ind)
                
                #-------------------- add --------------------
                img_names = os.listdir(os.path.join(self.root_path, record.path))
                img_names = sorted([os.path.splitext(img_name)[0]  for img_name in img_names])
                new_seg_ind = [int(img_names[ind-1]) for ind in new_seg_ind]
                # try:
                #     new_seg_ind = [int(img_names[ind]) for ind in new_seg_ind]
                # except:
                #     print(f"new_indices:{new_seg_ind}, files:{len(img_names)}, end_frame:{record.end_frame}, path:{record.path}, indices:{indices}")
    ...
    """
                ...
                # for pil_img in seg_imgs:
                #     # 2. PIL (RGB) -> NumPy (RGB)
                #     img_np_rgb = np.array(pil_img)
                    
                #     # 3. NumPy (RGB) -> OpenCV (BGR)
                #     img_np_bgr = cv2.cvtColor(img_np_rgb, cv2.COLOR_RGB2BGR)
                #     """
                #     dist_type可选 : 'CS', 'CC', 'BW', 'GNC', 'GB', 'JPEG'
                #     dist_level可选:  1,2,3,4,5  五个等级
                #     """
                #     dist_param = get_distortion_parameter('GB', 1)

                #     dist_function = get_distortion_function('GB')

                #     # 4. 应用失真
                #     # 这里会调用你指定的 color_saturation(img_np_bgr, 0.4)
                #     distorted_bgr = dist_function(img_np_bgr, dist_param)
                    
                #     # 5. OpenCV (BGR) -> NumPy (RGB)
                #     distorted_rgb = cv2.cvtColor(distorted_bgr, cv2.COLOR_BGR2RGB)
                    
                #     # 6. NumPy (RGB) -> PIL Image
                #     distorted_pil = Image.fromarray(distorted_rgb)
                    
                #     distorted_seg_imgs.append(distorted_pil)

                # images.extend(distorted_seg_imgs)
                images.extend(seg_imgs)
...
class VideoDataSetLMDB(data.Dataset):
    ...
    def __init__(self, datadir, db_name, num_groups=16, frames_per_group=1, sample_offset=0, num_clips=1,
                 ...
                 image_tmpl='{:03d}.jpg', transform=None, is_train=True, test_mode=False,
